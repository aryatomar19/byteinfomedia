  ...
  return (
    ...
    >
      ...
      <div className="dm-container relative">
        ...
        <div className="dm-results-stats__grid grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3 lg:gap-4">
          {metrics.map((metric, index) => (
            <div
              key={metric.label}
              className="dm-results-stats__card rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-4 text-center sm:px-5 sm:py-5"
            >
              <DmAnimatedCounter
                stat={metric}
                index={index}
                className="[&_p:first-child]:text-3xl [&_p:first-child]:sm:text-4xl [&_p:first-child]:lg:text-[2.5rem] [&_p:first-child]:text-[#FF6B35] [&_p:last-child]:mt-1.5 [&_p:last-child]:text-[0.65rem] [&_p:last-child]:sm:text-xs"
              />
            </div>
          ))}
        </div>
