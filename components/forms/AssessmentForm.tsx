...
export type AssessmentFormProps = {
  ...
  compact?: boolean;
  /** Spacious layout for the dedicated consultation request form. */
  comfortable?: boolean;
};
...
export function AssessmentForm({
  ...
  const comfortableInputClass =
    "h-[58px] min-h-[58px] px-[18px] py-4 text-sm leading-normal rounded-[15px]";
  ...
  const inputClass = dark
    ? cn(
        ...
        comfortable ? comfortableInputClass : compact ? compactInputClass : "px-4 py-3.5",
      )
    : cn(
  ...
  const labelClass = cn(
    comfortable
      ? "text-[0.8125rem] font-bold uppercase tracking-[1px] leading-none"
      : "text-[0.6875rem] font-bold uppercase tracking-[0.12em] leading-none",
    dark ? "text-white/80" : "text-[#0A0F1C]",
  );
  const labelGridClass = cn("grid", comfortable ? "gap-2.5" : compact ? "gap-0.5" : "gap-2");
  ...
  return (
    <form
      ...
      className={cn(
        ...
        comfortable ? "p-8 sm:p-10" : compact ? "p-0" : "p-6 sm:p-8",
        dark
    ...
    >
      ...
      <div className={cn("relative", comfortable ? "mb-8" : compact ? "mb-2" : "mb-6")}>
        {showPriorityBadge && (
          <div
            className={cn(
              ...
              comfortable
                ? "mb-4 px-3 py-1 text-xs tracking-[0.14em]"
                : compact
                  ? "mb-1.5 py-0.5"
                  : "mb-4 px-3 py-1 text-xs tracking-[0.14em]",
            )}
        ...
        <p
          className={cn(
            comfortable ? "mt-3 text-sm leading-6" : compact ? "mt-1 text-sm leading-5" : "mt-3 text-sm leading-6",
      ...
      <div
        className={cn(
          "relative grid",
          comfortable ? "grid-cols-1 gap-5" : compact ? "grid-cols-1 gap-2" : "gap-4 sm:grid-cols-2",
        )}
      >
        ...
        <label
          className={cn(labelGridClass, !compact && !comfortable && "sm:col-span-2")}
        ...
        >
          ...
          <textarea
            id={`${idPrefix}-message`}
            name="message"
            rows={comfortable ? 5 : compact ? 2 : 4}
            placeholder="Tell us about your cloud, DevOps, security, or AI requirements."
            className={cn(
              inputClass,
              comfortable && "h-[130px] min-h-[120px] max-h-[140px] resize-none",
              compact && !comfortable && "h-[72px] min-h-[60px] max-h-[80px] resize-none py-2 leading-snug",
            )}
          />
        </label>
      </div>
      ...
      <button
        ...
        className={cn(
          ...
          comfortable ? "mt-6 py-4" : compact ? "mt-2 py-2" : "mt-6 py-4",
        )}
      >
